...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/payment-stages/:id/confirm-payment", authenticateToken, upload.single('proofFile'), async (req: AuthRequest, res) => {
    try {
      ...
      if (!stage) return res.status(404).json({ message: "Etapa de pago no encontrada" });
      ...
      let proofFileUrl = stage.proofFileUrl;
      if (file) {
        proofFileUrl = `uploads/proofs/${Date.now()}-${file.originalname}`;
      }
      ...
      await db.update(paymentStages).set({
        status: 'pending_verification',
        paymentMethod: paymentMethod || stage.paymentMethod,
        proofFileUrl: proofFileUrl,
        updatedAt: new Date()
      ...
      try {
        await sendPaymentProofNotificationToAdmin(
          ...
          file ? `Archivo: ${file.originalname}` : "Sin archivo adjunto"
        );
        ...
        console.error("Error enviando notificaciones de pago:", emailError);
      }

      res.json({ message: "Comprobante de pago recibido y notificado" });
    } catch (error) {
      console.error("Error al confirmar pago de etapa:", error);
    ...
    try {
      ...
      if (companyInfo[0]) {
        ...
        if (!existingInvoice[0]) {
          // Get company info again to get latest sequence
          ...
          const boletaPrefix = currentCompany.boletaPrefix || '001-001';
          const currentSequence = currentCompany.boletaSequence || 1;
          boletaNumber = `${boletaPrefix}-${String(currentSequence).padStart(7, '0')}`;

          // Check again if this number was already used by another invoice (concurrency check)
          const [duplicateNumberInvoice] = await db
            .select()
            .from(invoices)
            .where(eq(invoices.invoiceNumber, boletaNumber))
            .limit(1);

          if (duplicateNumberInvoice) {
            console.log(`⚠️ Invoice number ${boletaNumber} already exists, skipping number increment and retrying logic...`);
            // We'll let it fail or handle it, but for now we try to avoid creating a second DB record with same number
            // Better: use the existing one if it belongs to the same stage
            if (duplicateNumberInvoice.paymentStageId === stageId) {
              newInvoice = duplicateNumberInvoice;
            } else {
              // This is a real collision on sequence, increment and try next turn or handle error
              return res.status(500).json({ message: "Error de secuencia de facturación. Por favor intente de nuevo." });
            }
          } else {
            // Get client billing info to create snapshot
            const clientInfo = await db
    ...
    try {
      ...
      await db
        ...
        .set({
          ...
          paymentMethodId: isNaN(parseInt(paymentMethod)) ? null : parseInt(paymentMethod),
      ...
      // Notify admin
      const adminUsers = await storage.getUsersByRole("admin");
      const selectedMethodName = paymentMethod === 'ueno' ? 'Ueno Bank' : (paymentMethod === 'mango' ? 'Mango (TU FINANCIERA)' : 'Transferencia Bancaria');
      
      for (const admin of adminUsers) {
        await storage.createNotification({
          userId: admin.id,
          title: "🧾 Comprobante de Factura Recibido",
          message: `El cliente ${req.user!.fullName} ha enviado un comprobante (${selectedMethodName}) para la factura #${invoice.invoiceNumber}.`,
          type: "info",
        });
      }
  ...
  app.get("/api/client/invoices/:id/download", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      const numeroALetras = (num: number): string => {
        ...
        // Centenas
        if (num >= 100) {
          letras += centenasArr[Math.floor(num / 100)] + ' ';
          num %= 100;
        }
      ...
      const montoEnLetras = `${numeroALetras(Math.round(totalAmount))} GUARANIES`;
      ...
      // Document information in the right box with background
      // Posición ajustada del cuadro - 10 puntos más a la izquierda
      const rightHeaderXNewHeader = pageWidth - 220;
      doc.rect(rightHeaderXNewHeader - 5, yPos + 10, 185, 65).fillColor('#f8fafc').fill();
      doc.rect(rightHeaderXNewHeader - 5, yPos + 10, 185, 65).strokeColor('#1e3a8a').lineWidth(1).stroke();

      const issueDate = invoice.createdAt ? new Date(invoice.createdAt).toLocaleString('es-PY', { hour12: true }) : 'N/A';
      doc.fontSize(14).fillColor('#1e3a8a').font('Helvetica-Bold').text('BOLETA ELECTRÓNICA', rightHeaderXNewHeader, yPos + 15);
      doc.fontSize(8).fillColor('#475569').font('Helvetica').text('IVA General 10% / IRE SIMPLE', rightHeaderXNewHeader, yPos + 32);
      doc.fontSize(10).fillColor('#1e293b').font('Helvetica-Bold').text(`BOLETA Nº: ${invoice.invoiceNumber || 'S/N'}`, rightHeaderXNewHeader, yPos + 48);
      doc.fontSize(8).fillColor('#475569').font('Helvetica').text(`Emisión: ${issueDate}`, rightHeaderXNewHeader, yPos + 62);
      ...
      // Primera columna
      doc.fontSize(7).fillColor('#475569').font('Helvetica-Bold').text('BOLETA Nº:', leftMargin + 8, yPos + 5);
      doc.fontSize(10).fillColor('#1e3a8a').font('Helvetica-Bold').text(invoice.invoiceNumber || 'S/N', leftMargin + 8, yPos + 20);

      // Segunda columna
      doc.fontSize(7).fillColor('#475569').font('Helvetica-Bold').text('FECHA Y HORA DE EMISIÓN:', leftMargin + contentWidth * 0.33 + 8, yPos + 5);
      doc.fontSize(9).fillColor('#000000').font('Helvetica-Bold').text(issueDate, leftMargin + contentWidth * 0.33 + 8, yPos + 20);

      // Tercera columna
      doc.fontSize(7).fillColor('#475569').font('Helvetica-Bold').text('TIPO:', leftMargin + contentWidth * 0.66 + 8, yPos + 5);
      doc.fontSize(10).fillColor('#000000').font('Helvetica-Bold').text(`MANUAL`, leftMargin + contentWidth * 0.66 + 8, yPos + 20);
      ...
      // Aviso de Firma Digital en Proceso
      doc.rect(leftMargin, yPos, contentWidth, 25).fillColor('#fff7ed').fill();
      doc.rect(leftMargin, yPos, contentWidth, 25).strokeColor('#ea580c').lineWidth(1).stroke();
      doc.fontSize(8).fillColor('#9a3412').font('Helvetica-Bold').text('FIRMA DIGITAL EN PROCESO - ESTE DOCUMENTO ES UNA CONSTANCIA DE PAGO INTERNA', leftMargin, yPos + 8, { align: 'center', width: contentWidth });
      yPos += 35;
      ...
      doc.fontSize(8).fillColor('#1e293b').font('Helvetica');
      doc.text(`Titular: ${company?.titularName || company?.companyName || 'JHONI FABIAN BENITEZ DE LA CRUZ'}`, leftMargin + 8, yPos + 25, { width: columnWidth - 16 });
      doc.text(`RUC: ${company?.ruc || '4220058-0'}`, leftMargin + 8, yPos + 42);
      doc.text(`Tel: ${company?.phone || '+595 985 990 046'}`, leftMargin + 8, yPos + 54);
      doc.text(`Email: ${company?.email || 'softwarepar.lat@gmail.com'}`, leftMargin + 8, yPos + 66, { width: columnWidth - 16 });
      doc.text(`Dirección: ${company?.address || 'Paraguay'}`, leftMargin + 8, yPos + 78, { width: columnWidth - 16 });
      ...
      doc.fontSize(8).fillColor('#1e293b').font('Helvetica');
      doc.text(`Nombre: ${invoice.clientLegalName || 'N/A'}`, clientBoxX + 8, yPos + 25, { width: columnWidth - 16 });
      doc.text(`${invoice.clientRuc ? (invoice.clientRuc.includes('-') ? 'RUC' : 'Documento_Extranjero') : 'RUC/CI'}: ${invoice.clientRuc || 'N/A'}`, clientBoxX + 8, yPos + 42);
      doc.text(`Tel: ${invoice.clientSnapshotPhone || 'N/A'}`, clientBoxX + 8, yPos + 54);
      doc.text(`Email: ${invoice.clientSnapshotEmail || 'N/A'}`, clientBoxX + 8, yPos + 66, { width: columnWidth - 16 });
      doc.text(`Dirección: ${invoice.clientSnapshotAddress || 'N/A'}`, clientBoxX + 8, yPos + 78, { width: columnWidth - 16 });

      yPos += boxHeight + 15;

      // ==> TABLA DE SERVICIOS <==
      const cantWidth_Manual = 40;
      const descWidth_Manual = 270;
      const precioWidth_Manual = 110;
      const totalWidth_Manual = 95;
      const exchangeRateValue_Manual = parseFloat(invoice.exchangeRateUsed || '1');
      const totalAmount_Manual = parseInt(invoice.totalAmount);

      doc.rect(leftMargin, yPos, contentWidth, 25).fillColor('#1e3a8a').fill();
      doc.fontSize(9).fillColor('#ffffff').font('Helvetica-Bold');
      doc.text('CANT', leftMargin + 5, yPos + 8);
      doc.text('DESCRIPCIÓN DEL SERVICIO', leftMargin + cantWidth_Manual + 10, yPos + 8);
      doc.text('PRECIO UNITARIO', leftMargin + cantWidth_Manual + descWidth_Manual + 10, yPos + 8);
      doc.text('TOTAL', leftMargin + cantWidth_Manual + descWidth_Manual + precioWidth_Manual + 20, yPos + 8);
      ...
      doc.fontSize(9).fillColor('#1e293b').font('Helvetica');
      doc.text('1', leftMargin + 15, yPos + 10);
      doc.text(invoice.description || "Servicio General", leftMargin + cantWidth_Manual + 10, yPos + 10, { width: descWidth_Manual - 20 });
      
      doc.fontSize(8).fillColor('#64748b');
      doc.text(`Proyecto: ${invoice.projectName || 'N/A'}`, leftMargin + cantWidth_Manual + 10, yPos + 22);
      doc.text(`TC: 1 USD = ${exchangeRateValue_Manual.toLocaleString('es-PY', { minimumFractionDigits: 2 })} PYG`, leftMargin + cantWidth_Manual + 10, yPos + 34);

      doc.fontSize(9).fillColor('#1e293b').font('Helvetica-Bold');
      doc.text(`USD ${parseFloat(invoice.amount).toFixed(2)}`, leftMargin + cantWidth_Manual + descWidth_Manual, yPos + 10, { width: precioWidth_Manual, align: 'center' });
      doc.text(`USD ${parseFloat(invoice.amount).toFixed(2)}`, leftMargin + cantWidth_Manual + descWidth_Manual + totalWidth_Manual + 15, yPos + 10, { width: totalWidth_Manual, align: 'center' });
      
      doc.fontSize(8).fillColor('#64748b').font('Helvetica');
      doc.text(`PYG ${parseInt(invoice.totalAmount).toLocaleString('es-PY')}`, leftMargin + cantWidth_Manual + descWidth_Manual, yPos + 22, { width: precioWidth_Manual, align: 'center' });
      doc.text(`PYG ${parseInt(invoice.totalAmount).toLocaleString('es-PY')}`, leftMargin + cantWidth_Manual + descWidth_Manual + totalWidth_Manual + 15, yPos + 22, { width: totalWidth_Manual, align: 'center' });

      yPos += 75;

      // ==> TOTALES <==
      const totalsWidth_Manual = 180;
      const totalsX_Manual = leftMargin + contentWidth - totalsWidth_Manual;
      
      doc.fontSize(9).fillColor('#475569').font('Helvetica');
      doc.text('Subtotal USD:', totalsX_Manual, yPos);
      doc.text(`USD ${parseFloat(invoice.amount).toFixed(2)}`, totalsX_Manual + 90, yPos, { width: 90, align: 'right' });

      yPos += 15;
      doc.text('Subtotal PYG:', totalsX_Manual, yPos);
      doc.text(`PYG ${parseInt(invoice.totalAmount).toLocaleString('es-PY')}`, totalsX_Manual + 90, yPos, { width: 90, align: 'right' });

      yPos += 15;
      doc.text('IVA (10%):', totalsX_Manual, yPos);
      const ivaAmount_Manual = Math.round(totalAmount_Manual / 11);
      doc.text(`PYG ${ivaAmount_Manual.toLocaleString('es-PY')}`, totalsX_Manual + 90, yPos, { width: 90, align: 'right' });

      yPos += 20;
      doc.rect(totalsX_Manual - 5, yPos - 5, totalsWidth_Manual + 5, 25).fillColor('#1e3a8a').fill();
      doc.fontSize(10).fillColor('#ffffff').font('Helvetica-Bold');
      doc.text('TOTAL:', totalsX_Manual, yPos + 2);
      doc.text(`PYG ${totalAmount_Manual.toLocaleString('es-PY')}`, totalsX_Manual + 60, yPos + 2, { width: totalsWidth_Manual - 65, align: 'right' });
      ...
      if (invoice.status === 'paid') {
        ...
        doc.fontSize(8).fillColor('#475569').font('Helvetica');
        const methodId = invoice.paymentMethodId;
        const methodDisplay = (methodId === 2 || methodId === 'ueno' || String(methodId).toLowerCase().includes('ueno')) ? 'Ueno Bank' : 'Mango (TU FINANCIERA)';
        doc.text(`Método: ${methodDisplay}`, leftMargin + 8, yPos + 20);
        doc.text(`Fecha: ${invoice.paidDate ? new Date(invoice.paidDate).toLocaleDateString('es-PY') : 'N/A'}`, leftMargin + 180, yPos + 20);
        doc.text(`Estado: PAGADO`, leftMargin + 350, yPos + 20);
        yPos += 55;
      }
      ...
      doc.fontSize(7).fillColor('#475569').font('Helvetica');
      doc.text('IVA General 10% / IRE SIMPLE', leftMargin, yPos, { align: 'center', width: contentWidth });
      yPos += 12;
      ...
      if (invoice.sifenQR) {
        try {
          const qrDataUrl = await QRCode.toDataURL(invoice.sifenQR, { margin: 1 });
          const qrBuffer = Buffer.from(qrDataUrl.split(',')[1], 'base64');
          doc.image(qrBuffer, qrX, qrY, { width: qrSize, height: qrSize });
        } catch (e) {
          console.error('Error QR:', e);
      ...
      textY += 20;
      const cdcFormatted = invoice.sifenCDC?.match(/.{1,4}/g)?.join(' ') || invoice.sifenCDC || 'N/A';
      doc.fontSize(10).font('Helvetica-Bold').text('CDC:', textX, textY);
      doc.fontSize(10).font('Helvetica').text(cdcFormatted, textX + 30, textY, { width: textWidth - 30 });
