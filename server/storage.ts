...
import { db } from "./db";
import { eq, desc, sql, ne, and, gte, isNull, lte, inArray, asc } from "drizzle-orm";
...
export interface IStorage {
  ...
  getCompanyBillingInfo(): Promise<CompanyBillingInfo[]>;
...
export class DatabaseStorage implements IStorage {
  ...
  async createInvoiceForProject(projectId: number, amountUsd: string, dueDate: Date): Promise<Invoice> {
    try {
      ...
      if (!company) throw new Error("No hay información de facturación configurada");

      // Increment sequence atomically and get new value
      const [updatedCompany] = await db.update(companyBillingInfo)
        .set({ boletaSequence: sql`${companyBillingInfo.boletaSequence} + 1` })
        .where(eq(companyBillingInfo.id, company.id))
        .returning();

      const nextSeqNum = updatedCompany.boletaSequence;
      const invoiceNumber = `001-001-${nextSeqNum.toString().padStart(7, '0')}`;
  ...
  async getClientBillingData(clientId: number): Promise<any> {
    try {
      // 1. Total del Proyecto (Suma de todos los precios de proyectos del cliente)
      const userProjects = await db
        .select({ price: projects.price })
        .from(projects)
        .where(eq(projects.clientId, clientId));
      
      const projectTotal = userProjects.reduce((sum, p) => sum + Number(p.price || 0), 0);

      // 2. Total Pagado (Suma de todas las facturas pagadas)
      ...
      // 3. Pagos Pendientes (Suma de todas las facturas con estado 'pending')
      ...
      const pendingPayments = pendingInvoices.reduce((sum, inv) => sum + Number(inv.amount || 0), 0);

      // 4. Próximo Vencimiento (Fecha de la factura pendiente más próxima)
  ...
  async getCompanyBillingInfo(): Promise<CompanyBillingInfo[]> {
    return await db.select().from(companyBillingInfo);
  }
